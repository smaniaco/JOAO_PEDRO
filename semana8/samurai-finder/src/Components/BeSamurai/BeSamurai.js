...
export class BeSamurai extends React.Component {
  ...
  createjob = async(event)=>{
    ...
    try{
      ...
      console.log(error)
      alert("Parece que tivemos algum problema. Verifique todos os campos, veja se sua o Título e Descrição não são pequenos e se a data é maior do que a data atual")
    }
    this.setState({title:"",
  description:"",
  price:"",
    ...
    <MainDiv>
      ...
      <PageGrid id="besamurai">
        ...
        <FormDiv>
          ...
          <CheckBoxes>
          <p>Pix</p>
          <input id="pix" onChange={()=>this.onChangePayment("pix")} type="checkbox"/>
          <p>Crédito</p>
          <input  id="credito" onChange={()=>this.onChangePayment("credito")} type="checkbox"/>
          <p>Débito</p>
          <input id="debito" onChange={()=>this.onChangePayment("debito")} type="checkbox"/>
          <p>boleto</p>
