...
export class Find extends React.Component {
  ...
  getAllLists = async()=>{
    let response
    try{
      response = await axios.get("https://labeninjas.herokuapp.com/jobs", {headers:{authorization:"c031e0dd-6176-4cbf-9978-49c392be9b8c"}})
    }
    catch(error){
  ...
  updateJob = async(item)=>{
    ...
    try{
      axios.post(`https://labeninjas.herokuapp.com/jobs/${item.id}`, body, {headers:{authorization:"c031e0dd-6176-4cbf-9978-49c392be9b8c"}})
    }
    catch(error){
  ...
  const filteredItems = this.state.list.filter((item)=>{
    ...
    if ((item.price >= this.state.minValue || this.state.minValue === "") && (item.price <= this.state.maxValue || this.state.maxValue === "")) {
      if ((item.title.toLowerCase().includes(this.state.TitleDescription.toLowerCase())) || item.description.includes(this.state.TitleDescription)){
  ...
  if (this.state.order === "Valor crescente"){
    ...
    filteredItems.sort((a, b) => (Number(a.dueDate) < Number(b.dueDate)) ? 1 : -1)
  }
  ...
  const itemsList = filteredItems.map((item)=>{
    ...
    return <Card.Grid  className="cardItem" key = {item.id}>
        <h5>{item.title}</h5>
       ...
       // <Button onClick={()=>this.deleteJob(item)}>Descontratar</Button>:
        ...
        {item.showDetails ? <Button onClick={()=>this.showDetails(item.id)}>Cancelar</Button> :
  ...
  return (
    <MainDiv>
        <InputsSearch>
          ...
          <input value={this.state.TitleDescription} onChange ={this.onChangeTitleDescription}
          />
